...
使用以下单行命令即可在全新的 Debian 12 服务器上完成所有部署工作。
...
-   脚本需要以 `root` 权限运行。
-   脚本会自动安装 `git`、`docker` 和 `docker-compose`，克隆您的项目，并启动服务。

部署成功后，您可以通过浏览器访问 `http://<你的服务器IP>:74674` 来使用 Netcup VPS Sentinel。

### 更新应用程序

当您需要更新代码时，只需在您的项目目录（例如 `~/netcup-sentinel`）中：
1.  运行 `git pull` 拉取最新的代码。
2.  再次进入目录 `cd ~/netcup-sentinel`
3.  运行 `docker-compose up --build -d` 重新部署。Docker 会自动使用最新的代码重建镜像并启动新的容器。
